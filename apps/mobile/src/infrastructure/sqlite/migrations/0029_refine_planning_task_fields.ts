...
export const refinePlanningTaskFields: Migration = {
  version: 29,
  name: "refine_planning_task_fields",
  statements: [
    "PRAGMA foreign_keys = OFF;",
    `CREATE TABLE IF NOT EXISTS planning_tasks_rebuilt (
    ...
    `INSERT INTO planning_tasks_rebuilt (
      id, farm_id, goal_id, place_id, title, notes, status, priority, planned_start_date,
      due_date, estimated_minutes, assigned_farmhand_id, instruction_photo_json, completion_notes,
      completed_at, source, template_key, sort_order, created_at, updated_at
    )
    SELECT
      ...
      due_date, estimated_minutes, assigned_farmhand_id, '[]', completion_notes,
      completed_at, source, template_key, sort_order, created_at, updated_at
    FROM planning_tasks;`,
    "DROP TABLE planning_tasks;",
    "ALTER TABLE planning_tasks_rebuilt RENAME TO planning_tasks;",
