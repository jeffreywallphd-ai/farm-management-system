...
- Canonical for: local mobile planning, roadmap, goal, subgoal, assignment-ready task, and task instruction foundation
...
- tasks with optional desired start dates and target completion dates;
- optional local task instruction voice memos and photos for work directions;
- local links from goals/tasks to farm notes, farm places, tracked items, organic records, and reports;
- optional local farmhand assignment references for future worker-facing workflows.
...
Goals and subgoals support long-term farm planning without requiring dense project-management screens. Desired start dates and target completion dates let farmers express timing without adding calendar automation. Tasks provide the lowest shared unit for both planning and assignment-ready work. Optional instruction voice memos and photos help farmers describe work in field-friendly ways while remaining local source attachments.
...
- desired start date and target completion date persistence;
- optional task instruction audio/photo persistence and recovery export;
