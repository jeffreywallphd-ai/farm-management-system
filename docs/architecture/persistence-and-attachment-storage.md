...
ADR-0012 adds local farm-event capture metadata and private audio/photo attachment references. The app now includes the first SQLite schema and repository boundary for event metadata and local attachment references, plus local voice memo recording, retained-file copy, playback, optional photo picking/capture, photo preview, retained-photo copy, previous-note photo review with an unavailable-file state, and a user-controlled ZIP recovery package containing metadata plus retained audio/photo files.

ADR-0013 adds local transcript-draft persistence for saved farm-note voice memos. Transcript drafts store text/status/model metadata locally and export with the farm-note media package when present. The current implementation uses a `whisper.rn` adapter and safely reports the model-missing state until the local Whisper model is available in a validated development build. Import/restore, sync state, structured AI extraction, publications, and authentication remain unimplemented.
