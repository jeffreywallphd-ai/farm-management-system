...
The first pilot must work without a live server. A worker must be able to capture supported farm events locally, review local history or timeline views, and understand that the data is stored on the device unless exported/backed up.
...
| Voice/photo farm-event capture | Required by ADR-0012 for the next farmer-shareable pilot; not yet implemented | Local source event retained with metadata and optional attachments |
...
| Export/backup | Required before meaningful reliance; manual-record recovery copy implemented in Phase 3; media recovery package required before capture reliance | User-controlled data retrieval/backup |
...
| AI interpretation of voice/photo captures | Deferred | Future draft-and-confirm behavior |
...
- Local voice/photo farm-event captures accepted by ADR-0012.
- Drafts from AI-assisted interpretation only in Mobile Pilot 2 or later.
- Source-capture provenance where needed to review retained voice/photo events.
...
- Voice/photo capture in ADR-0012 does not imply transcription, object recognition, AI interpretation, server transfer, or upload.
