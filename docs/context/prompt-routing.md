...
| Implement local inventory catalog, linked inventory purchase notes, or material usage reporting | `product-and-domain`, `mobile-field-capture`, `offline-sync`, `privacy-and-sharing`, `testing-and-diagnostics`; add `organic-certification` when organic fields are affected | ADR-0007, ADR-0009, ADR-0010, ADR-0011, ADR-0012, `docs/domain/inventory-and-reconciliation-rules.md`, `docs/domain/mobile-pilot-1-operational-records.md`, `docs/operations/mobile-pilot-data-safety-requirements.md` | Authorized as local materials/equipment catalog with guided category/common-item setup, farmer-entered source and amount-on-hand context, private purchase-oriented farm notes linked to catalog items with optional photos, and reporting over confirmed material-use records only; equipment-use records, purchase orders, accounting, automatic stock receipts, sync, AI extraction, and compliance decisions remain deferred |
...
| Implement saved-note on-device transcript draft | `ai-assisted-recording`, `mobile-field-capture`, `privacy-and-sharing`, `dependency-and-technology-selection`, `testing-and-diagnostics` | ADR-0013, ADR-0003, ADR-0004, ADR-0007, ADR-0012, AI domain/architecture docs, persistence/attachment storage | Authorized only as local draft text tied to source audio; model download may fetch the approved Whisper model only; no structured extraction, cloud transcription provider, sync, auth, or operational effects |
| Implement organic certification readiness phase | `organic-certification`, `product-and-domain`, `mobile-field-capture`, `offline-sync`, `privacy-and-sharing`, `testing-and-diagnostics` | ADR-0014, `docs/product/organic-certification-readiness.md`, `docs/domain/organic-certification-rules.md`, `docs/architecture/organic-certification-architecture.md`, official USDA/eCFR sources for the phase | Authorized as local evidence/reporting support only; no certifier submission, legal determination, sync, auth, cloud, analytics, or automatic AI extraction |
| Implement local farm planning or certification planning templates | `planning`, `product-and-domain`, `mobile-field-capture`, `offline-sync`, `privacy-and-sharing`, `testing-and-diagnostics`; add `organic-certification` when certification screens/tasks are affected | ADR-0015, `docs/product/farm-planning-roadmapping.md`, `docs/domain/farm-planning-rules.md`, `docs/architecture/planning-architecture.md`; plus organic docs for certification planning | Authorized as local goals/subgoals/periods/tasks only; no cloud calendar, notifications, auth, worker accounts, sync, automatic AI task generation, or workflow engine |
| Implement local farmhand management, schedules, or farmhand task assignment | `planning`, `product-and-domain`, `mobile-field-capture`, `privacy-and-sharing`, `offline-sync`, `testing-and-diagnostics` | ADR-0015, `docs/product/farmhand-management.md`, `docs/domain/farmhand-management-rules.md`, `docs/architecture/farmhand-management-architecture.md`, privacy docs | Authorized as local private farmhand records, schedules, and task-assignment views only; no payroll, timeclock, worker accounts, messaging, notifications, companion app, sync, or auth |
| Implement local farm map settings, farm location, GPS/geocoding, GeoJSON geometry, or map previews | `gis-map-and-geometry`, `mobile-field-capture`, `offline-sync`, `privacy-and-sharing`, `dependency-and-technology-selection`, `testing-and-diagnostics` | ADR-0016, `docs/product/basic-gis-and-farm-location.md`, `docs/domain/gis-map-and-geometry-rules.md`, `docs/architecture/gis-map-and-geometry-architecture.md`, dependency standards | Authorized as local farm-owned map/geometry data only; no cloud sync, auth, analytics, server GIS APIs, automatic upload, or required online basemap |
| Implement structured AI interpretation of voice/photo capture | `ai-assisted-recording`, `mobile-field-capture`, `product-and-domain`, `privacy-and-sharing` as needed | ADR-0003, ADR-0007, ADR-0012, AI domain/architecture docs, validation plan | Deferred until capture-first evidence and later scope/ADR review |
