...
| Add local voice/photo farm-event capture | ADR-0012, ADR-0001, ADR-0004, Mobile Pilot 1 scope, persistence/attachment storage, mobile pilot data-safety requirements | Product/domain/architecture/operations/context | Local file retention + event timeline + recovery package + privacy tests | Capture is local/private source data; do not add AI interpretation, upload, sync, auth, analytics, or sharing |
| Add saved-note on-device transcript draft | ADR-0013, ADR-0003, ADR-0004, ADR-0012, AI boundaries, persistence/attachment storage, dependency standards | ADR/readiness/domain/architecture/operations/context/app README | Transcript use-case + persistence + export + model download/status + manual device tests | Draft text only; original audio remains source of truth; model download is limited to approved model files; no structured records, cloud transcription, sync, auth, analytics, or sharing |
