...
Phase 3 now implements the complete manual Mobile Pilot 1 core: manual `HarvestRecorded`, `MaterialUseRecorded`, and `InventoryCountRecorded` creation; unified local activity history/detail; local Inventory Management for guided farm input/material and equipment catalog setup, farmer-entered source and amount-on-hand context, purchase-oriented farm-note entry points, and material usage reporting; and a versioned JSON recovery-copy export for farm setup/reference data, inventory catalog details, and all implemented manual records.
...
| Local inventory catalog for materials/equipment | Included |
| Inventory material/equipment purchase farm-note entry points | Included as private farm-event notes with optional photos |
| Material usage report from confirmed material-use records | Included |
...
| Equipment-use, equipment-cleaning, maintenance, or movement records | Candidate later manual workflow after discovery |
| Purchase orders, accounting records, inventory receipts, reconciled stock ledgers, or automatic stock adjustment from catalog source/amount fields or purchase notes | Candidate later workflow after discovery |
